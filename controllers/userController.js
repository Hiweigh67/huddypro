    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>Don't Share this code with anyone, not even our support team.</p>
        </div>
        <div class="footer">
...
const sendMailVerify = async (req,res)=>{
    try {
        ...
        const mailOptions = {
            from: 'Huddypro Files <kitahope@yandex.ru>',
            to: [`${req.body.email}`], // Update recipients as an array for multiple recipients
            subject: 'Verification',
            text: "Don't share this email with any one, not even a worker at HUDDYPRO FILES",
            html: htmlMsg(secCode,req.body.LName),
            replyTo: "kitahope@yandex.ru"
        };
        const ck = await db.readRow({email:req.body.email},'huddypro','admin');
        const cj = await db.readRow({email:req.body.email},'huddypro','users');
